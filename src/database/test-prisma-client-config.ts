...
try {
  ...
  if (error.message.includes('Cannot find module')) {
    console.error('   💡 Verifique se o arquivo client.ts existe em src/database/')
  }
  process.exit(1)
}
...
if (databaseUrl) {
  ...
  if (databaseUrl.startsWith('file:')) {
    ...
    const fs = require('fs')
    const dir = path.dirname(dbPath.startsWith('./') ? dbPath.substring(2) : dbPath)
    
    if (fs.existsSync(dir) || dir === '.' || dir === '') {
      console.log('   ✅ Diretório do banco existe ou será criado automaticamente')
    } else {
      console.log(`   ⚠️ Diretório ${dir} não existe - será criado no primeiro push`)
    }
  }
} else {
  console.error('   ❌ DATABASE_URL não encontrada')
  console.error('   💡 Crie um arquivo .env com DATABASE_URL="file:./src/database/dev.db"')
  process.exit(1)
}
...
if (fs.existsSync(schemaPath)) {
  ...
  if (schemaContent.includes('provider = "sqlite"')) {
    ...
    console.log('   ⚠️ Provider SQLite não encontrado - verifique a configuração')
  }
  
} else {
  console.error(`   ❌ Schema não encontrado em: ${schemaPath}`)
  console.error('   💡 Crie o arquivo schema.prisma na pasta src/database/prisma/')
  process.exit(1)
}
...
try {
  ...
  const foundModels = expectedModels.filter(model => clientKeys.includes(model))
  
  if (foundModels.length > 0) {
    console.log(`   ✅ Models encontrados: ${foundModels.join(', ')}`)
  } else {
    console.log('   ⚠️ Nenhum model encontrado - cliente pode não ter sido gerado')
