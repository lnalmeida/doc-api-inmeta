...
export class EmployeeService {
  ...
  async create(data: CreateEmployeeDto): Promise<Employee> {
    ...
    const existingEmployee =
      await this.employeeRepository.findEmployeeByCpf(cleanedCpf);
    if (existingEmployee) {
      throw new ConflictException(
        `Um colaborador com o CPF "${data.cpf}" já existe.`,
      );
    }
    data.cpf = cleanedCpf;
  ...
  async update(id: string, data: UpdateEmployeeDto): Promise<Employee> {
    ...
    if (data.cpf && data.cpf !== existingEmployee.cpf) {
      const employeeWithSameCpf =
        await this.employeeRepository.findEmployeeByCpf(data.cpf);
