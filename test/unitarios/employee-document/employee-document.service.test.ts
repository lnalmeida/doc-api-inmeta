...
import { DocumentStatus, Employee, DocumentType, EmployeeDocument } from '@prisma/client';
import { PaginationResult } from '../../../src/common/types/pagination.types';
import { EmployeeDocumentWithDocumentType, EmployeeDocumentWithRelations } from '../../../src/common/types/EmployeeDocumentTypes'; // Ajuste o caminho se necessário
...
describe('EmployeeDocumentsService', () => {
  ...
  describe('atribuirTiposDeDocumento', () => {
    it('deve atribuir novos tipos de documentos a um colaborador com sucesso', async () => {
      const assignDto = { employeeId: mockEmployee.id, documentTypeIds: [mockDocumentTypeCPF.id, mockDocumentTypeRG.id] };
      ...
      expect(mockEmployeeRepository.findEmployeeById).toHaveBeenCalledWith(mockEmployee.id);
      expect(mockDocumentTypeRepository.findAllDocumentTypes).toHaveBeenCalled();
      expect(mockEmployeeDocumentRepository.findEmployeeDocumentsByEmployeeId).toHaveBeenCalledWith(mockEmployee.id);
      expect(mockEmployeeDocumentRepository.assignDocumentTypes).toHaveBeenCalledWith(
        mockEmployee.id,
        [mockDocumentTypeCPF.id, mockDocumentTypeRG.id] 
      );
    });

    it('deve lançar NotFoundException se o colaborador não for encontrado', async () => {
      const assignDto = { employeeId: 'non-existent-id', documentTypeIds: [mockDocumentTypeCPF.id] };

      mockEmployeeRepository.findEmployeeById.mockResolvedValue(null); 

      await expect(service.assignDocumentTypes(assignDto)).rejects.toThrow(NotFoundException);
      expect(mockEmployeeRepository.findEmployeeById).toHaveBeenCalledWith(assignDto.employeeId);
      expect(mockDocumentTypeRepository.findAllDocumentTypes).not.toHaveBeenCalled(); 
    });

    it('deve lançar BadRequestException se algum tipo de documento não existir', async () => {
      const assignDto = { employeeId: mockEmployee.id, documentTypeIds: [mockDocumentTypeCPF.id, 'non-existent-doc-type'] };

      mockEmployeeRepository.findEmployeeById.mockResolvedValue(mockEmployee);
      mockDocumentTypeRepository.findAllDocumentTypes.mockResolvedValue({ // Apenas CPF existe
        data: [mockDocumentTypeCPF],
        total: 1, page: 1, limit: 10, totalPages: 1
      });

      await expect(service.assignDocumentTypes(assignDto)).rejects.toThrow(BadRequestException);
      expect(mockDocumentTypeRepository.findAllDocumentTypes).toHaveBeenCalled();
    });
    ...
    it('deve lançar ConflictException se um tipo de documento já estiver atribuído ao colaborador', async () => {
      ...
      await expect(service.assignDocumentTypes(assignDto)).rejects.toThrow(ConflictException);
      expect(mockEmployeeDocumentRepository.findEmployeeDocumentsByEmployeeId).toHaveBeenCalledWith(mockEmployee.id);
      expect(mockEmployeeDocumentRepository.assignDocumentTypes).not.toHaveBeenCalled(); // Não deve chamar o repositório para criar
    });
  });

  describe('desatribuirTiposDeDocumento', () => {
    it('deve desatribuir tipos de documentos de um colaborador com sucesso', async () => {
      const unassignDto = { employeeId: mockEmployee.id, documentTypeIds: [mockDocumentTypeCPF.id] };

      mockEmployeeRepository.findEmployeeById.mockResolvedValue(mockEmployee);
      mockEmployeeDocumentRepository.unassignDocumentTypes.mockResolvedValue(undefined); // delete retorna void

      await service.unassignDocumentTypes(unassignDto);

      expect(mockEmployeeRepository.findEmployeeById).toHaveBeenCalledWith(mockEmployee.id);
      expect(mockEmployeeDocumentRepository.unassignDocumentTypes).toHaveBeenCalledWith(
        mockEmployee.id,
        unassignDto.documentTypeIds
      );
    });

    it('deve lançar NotFoundException se o colaborador para desatribuir não for encontrado', async () => {
      const unassignDto = { employeeId: 'non-existent-id', documentTypeIds: [mockDocumentTypeCPF.id] };

      mockEmployeeRepository.findEmployeeById.mockResolvedValue(null);

      await expect(service.unassignDocumentTypes(unassignDto)).rejects.toThrow(NotFoundException);
      expect(mockEmployeeRepository.findEmployeeById).toHaveBeenCalledWith(unassignDto.employeeId);
      expect(mockEmployeeDocumentRepository.unassignDocumentTypes).not.toHaveBeenCalled();
    });
  });

  // --- Testes para submitDocument() ---
  describe('enviarDocumento', () => {
    const submitDto = { employeeId: mockEmployee.id, documentTypeId: mockDocumentTypeCPF.id };
    ...
    it('deve registrar o envio de um documento com sucesso', async () => {
      mockEmployeeRepository.findEmployeeById.mockResolvedValue(mockEmployee);
      mockDocumentTypeRepository.findDocumentTypeById.mockResolvedValue(mockDocumentTypeCPF);
      mockEmployeeDocumentRepository.findByEmployeeIdAndDocumentTypeId.mockResolvedValue(mockEmployeeDocumentCPF_Pending); // Documento pendente
      mockEmployeeDocumentRepository.submitDocument.mockResolvedValue(submittedDocument);

      const result = await service.submitDocument(submitDto);

      expect(result.status).toBe(DocumentStatus.SUBMITTED);
      expect(mockEmployeeRepository.findEmployeeById).toHaveBeenCalledWith(submitDto.employeeId);
      expect(mockDocumentTypeRepository.findDocumentTypeById).toHaveBeenCalledWith(submitDto.documentTypeId);
      expect(mockEmployeeDocumentRepository.findByEmployeeIdAndDocumentTypeId).toHaveBeenCalledWith(submitDto.employeeId, submitDto.documentTypeId);
      expect(mockEmployeeDocumentRepository.submitDocument).toHaveBeenCalledWith(submitDto.employeeId, submitDto.documentTypeId, submitDto);
    });

    it('deve lançar NotFoundException se o colaborador não for encontrado ao enviar documento', async () => {
      mockEmployeeRepository.findEmployeeById.mockResolvedValue(null);

      await expect(service.submitDocument(submitDto)).rejects.toThrow(NotFoundException);
      expect(mockEmployeeRepository.findEmployeeById).toHaveBeenCalledWith(submitDto.employeeId);
      expect(mockDocumentTypeRepository.findDocumentTypeById).not.toHaveBeenCalled();
    });
    ...
    it('deve lançar NotFoundException se o tipo de documento não for encontrado ao enviar documento', async () => {
      ...
      await expect(service.submitDocument(submitDto)).rejects.toThrow(NotFoundException);
      expect(mockDocumentTypeRepository.findDocumentTypeById).toHaveBeenCalledWith(submitDto.documentTypeId);
      expect(mockEmployeeDocumentRepository.findByEmployeeIdAndDocumentTypeId).not.toHaveBeenCalled();
    });

    it('deve lançar NotFoundException se a vinculação do documento não for encontrada (não atribuído)', async () => {
      mockEmployeeRepository.findEmployeeById.mockResolvedValue(mockEmployee);
      mockDocumentTypeRepository.findDocumentTypeById.mockResolvedValue(mockDocumentTypeCPF);
      mockEmployeeDocumentRepository.findByEmployeeIdAndDocumentTypeId.mockResolvedValue(null); // Não vinculado

      await expect(service.submitDocument(submitDto)).rejects.toThrow(NotFoundException);
      expect(mockEmployeeDocumentRepository.findByEmployeeIdAndDocumentTypeId).toHaveBeenCalledWith(submitDto.employeeId, submitDto.documentTypeId);
      expect(mockEmployeeDocumentRepository.submitDocument).not.toHaveBeenCalled();
    });

    it('deve lançar ConflictException se o documento já estiver ENVIADO', async () => {
      mockEmployeeRepository.findEmployeeById.mockResolvedValue(mockEmployee);
      mockDocumentTypeRepository.findDocumentTypeById.mockResolvedValue(mockDocumentTypeCPF);
      mockEmployeeDocumentRepository.findByEmployeeIdAndDocumentTypeId.mockResolvedValue(mockEmployeeDocumentCPF_Submitted); // Já enviado

      await expect(service.submitDocument(submitDto)).rejects.toThrow(ConflictException);
      expect(mockEmployeeDocumentRepository.findByEmployeeIdAndDocumentTypeId).toHaveBeenCalledWith(submitDto.employeeId, submitDto.documentTypeId);
      expect(mockEmployeeDocumentRepository.submitDocument).not.toHaveBeenCalled();
    });
  });
  ...
  describe('obterStatusDocumentacaoColaborador', () => {
    it('deve retornar o status de documentação de um colaborador', async () => {
      ...
      mockEmployeeRepository.findEmployeeById.mockResolvedValue(mockEmployee);
      mockEmployeeDocumentRepository.findEmployeeDocumentsByEmployeeId.mockResolvedValue(docsWithRelations);
      ...
      expect(result.documents[0].documentTypeName).toBe(mockDocumentTypeCPF.name);
      expect(mockEmployeeRepository.findEmployeeById).toHaveBeenCalledWith(employeeId);
      expect(mockEmployeeDocumentRepository.findEmployeeDocumentsByEmployeeId).toHaveBeenCalledWith(employeeId);
    });
    ...
    it('deve lançar NotFoundException se o colaborador não for encontrado', async () => {
      ...
      await expect(service.getEmployeeDocumentStatus(employeeId)).rejects.toThrow(NotFoundException);
      expect(mockEmployeeRepository.findEmployeeById).toHaveBeenCalledWith(employeeId);
      expect(mockEmployeeDocumentRepository.findEmployeeDocumentsByEmployeeId).not.toHaveBeenCalled();
    });
  });
